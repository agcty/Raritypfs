...
var post_collection_exports = {};
__export(post_collection_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(post_collection_exports);
...
async function fetchToken(id) {
  try {
    const res = await fetch(`https://cloudflare-ipfs.com/ipfs/QmdJ8S7YfZmXQJYdieyHJhNpAUnqQ8KEQsgZ4EAdwYk7tx/${id}`);
...
var handler = async (event, context) => {
  ...
  for (let x2 = 1; x2 < 3; x2++) {
    const tmpList = [];
    for (let i2 = x2 * 20; i2 < x2 * 20 + 20; i2++) {
      tmpList.push(fetchToken(i2));
    }
...
async function traitsListing(tokenTraits) {
  let traitsTypes = [];
  let traitsValue = [];
  tokenTraits.forEach((trait) => trait.attributes.forEach((element) => {
    traitsTypes.push(element.trait_type);
    traitsValue.push(element.value);
  }));
  const map = traitsValue.reduce((acc, e2) => acc.set(e2, (acc.get(e2) || 0) + 1), /* @__PURE__ */ new Map());
  const uniqueTraitsValues = Array.from(map.entries());
  uniqueTraitsValues.sort(function(a, b) {
    return a[1] - b[1];
  });
  uniqueTraitsValues.forEach((element) => {
    collectionTraits[`${element[0]}`] = (100 * element[1] / supply).toFixed(2);
  });
